 ...
 *
 * PROPOSAL / REFERENCE IMPLEMENTATION. Nothing imports this yet. It lives in
 * infra/ beside the template that provisions the pool so the wire protocol and
 * the CloudFormation stay in one review.
 *
...
/**
 * Public identifiers, not secrets — a user pool id and an app client id with
 * no client secret are designed to ship in a browser bundle. They are filled
 * in from the CloudFormation stack outputs after the one-time apply. See
 * infra/APPLY.md.
 */
export const POOL = {
...
async function call(target, body) {
  ...
  try {
    ...
    // fetch() rejects only on network/CORS failure — never on a 4xx.
